...
Status WritableDB::Remove(const std::string& db_path)
{
	//获取写锁
	{
		...
		for(size_t i = 0; i < file_names.size(); ++i)
		{
			...
			DbMetaData bd;
			s = DbMetaFile::Read(db_path.c_str(), fn.str, bd);
			if(s != OK)
			{
				return s;
			}
...
{	
	...
	{
		m_delete_dbmeta_files.push_back(file_infos[i]);
	}

	DbMetaData bld;
	s = DbMetaFile::Read(m_path.c_str(), file_infos.back().str, bld);
	if(s != OK)
	{
		return s;
	}
	...
	{		
		{
			...
			if(m_delete_dbmeta_files.empty()) 
			{
				return ERR_NOMORE_DATA;
			}
			clean_filename = m_delete_dbmeta_files.front();
		}
		...
		}
		{
			...
			if(!m_delete_dbmeta_files.empty()) 
			{
				m_delete_dbmeta_files.pop_front();
			}
		}
	}
...
Status WritableDB::WriteDbMeta()
{
...

	{
		...
		assert(m_next_dbmeta_fileid < MAX_FILEID);
	...
	Status s = DbMetaFile::Write(m_path.c_str(), dbmeta_filename, bd);
	if(s != OK)
	{
		//LogWarn(msg_fmt, ...);
		return s;
	...
	if(dbmeta_fileid != MIN_FILEID)
	{
		...
		{
		std::lock_guard<std::mutex> lock(m_mutex);
		m_delete_dbmeta_files.push_back(name);
		}
...
Status WritableDB::CreateBucket(const std::string& bucket_name, BucketPtr& bptr)
{
	...
	((WritableEngine*)m_engine)->NotifyWriteDbMeta(shared_from_this());
	return OK;
}
...
void WritableDB::WriteDbMetaData(DbMetaData& bd)
{
	m_bucket_rwlock.ReadLock();
	BucketSnapshotPtr bs_ptr = m_bucket_snapshot;
	m_bucket_rwlock.ReadUnlock();
